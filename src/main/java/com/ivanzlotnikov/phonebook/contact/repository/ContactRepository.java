...
/**
 * Репозиторий для работы с сущностью {@link Contact}. Предоставляет методы для выполнения операций
 * CRUD и пользовательских запросов к базе данных контактов с оптимизацией загрузки связанных
 * сущностей.
 */
@Repository
public interface ContactRepository extends JpaRepository<Contact, Long> {
     ...
     * Загружает департамент вместе с контактом для избежания N+1 проблемы. Поиск выполняется по
     * всем трем полям ФИО. EntityGraph загружает все коллекции телефонов одним запросом.
     *
     * @param query    поисковый запрос
     * @param pageable параметры пагинации и сортировки
     * @return страница найденных контактов с департаментами и телефонами
     */
    @EntityGraph("Contact.withPhonesAndDepartment")
    @Query("""
        SELECT c FROM Contact c
        WHERE LOWER(CONCAT(c.lastName, ' ', c.firstName, ' ', COALESCE(c.middleName, ''))) 
     ...
     */
    boolean existsByLastNameAndFirstNameAndMiddleNameAndPosition(String lastName, String firstName,
    ...
    /**
     * Получает все контакты с загрузкой департаментов и всех телефонов. Использует отдельный запрос
     * для подсчета общего количества записей. EntityGraph оптимизирует загрузку, избегая N+1
     * проблемы.
     *
     * @param pageable параметры пагинации и сортировки
    ...
    /**
     * Находит контакты по списку идентификаторов департаментов. Загружает департаменты и все
     * телефоны вместе с контактами.
     *
    ...
    @Query("""
        ...
        """)
    Page<Contact> findByDepartmentIdInWithDepartment(
        @Param("departmentIds") List<Long> departmentIds, Pageable pageable);

    /**
     * Выполняет поиск контактов по ФИО в указанных департаментах. Комбинированный поиск с
     * фильтрацией по любому полю ФИО и департаментам. EntityGraph загружает все телефоны для
     * избежания N+1.
     *
    ...
    @Query("""
          ...
          """)
    Page<Contact> findByNameAndDepartmentIds(@Param("query") String query,
        @Param("departmentIds") List<Long> departmentIds, Pageable pageable);
